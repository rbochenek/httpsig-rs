...
use k256::{PublicKey as EcdsaSecp256k1PublicKey, SecretKey as EcdsaSecp256k1SecretKey};
use p256::NistP256;
use p384::NistP384;
...
pub enum SecretKey {
    ...
    /// ECDSA secp256k1
    EcdsaSecp256k1(EcdsaSecp256k1SecretKey),
}
...
impl SecretKey {
        ...
        let (algorithm_name, sk_bytes) = match pki.algorithm.oid.to_string().as_ref() {
            ...
            algorithm_oids::EC => {
                ...
                let sk_bytes = sec1::EcPrivateKey::try_from(pki.private_key)
                    .map_err(|e| {
                        HttpSigError::ParsePrivateKeyError(format!(
                            "Error decoding EcPrivateKey: {e}"
                        ))
                    })?
                    .private_key;
    ...
    pub fn public_key(&self) -> PublicKey {
        match &self {
            ...
            Self::EcdsaSecp256k1(key) => PublicKey::EcdsaSecp256k1(key.public_key()),
        }
    }
}
    ...
    fn sign(&self, data: &[u8]) -> HttpSigResult<Vec<u8>> {
        match &self {
            ...
            Self::EcdsaSecp256k1(sk) => {
                ...
                sig.extend_from_slice(signature.to_bytes().as_slice());
...
impl PublicKey {
    ...
    pub fn from_bytes(alg: AlgorithmName, bytes: &[u8]) -> HttpSigResult<Self> {
        match alg {
            ...
            AlgorithmName::EcdsaSecp256k1 => {
                debug!("Read EcdsaSecp256k1 public key");
                let pk = k256::PublicKey::from_sec1_bytes(bytes)
                    .map_err(|e| HttpSigError::ParsePublicKeyError(e.to_string()))?;
                Ok(Self::EcdsaSecp256k1(pk))
            }
        ...
        let (algorithm_name, pk_bytes) =
            match spki_ref.algorithm.oid.to_string().as_ref() {
                ...
                algorithm_oids::EC => {
                    ...
                    let pk_bytes = spki_ref.subject_public_key.as_bytes().ok_or(
                        HttpSigError::ParsePublicKeyError("Invalid public key".to_string()),
                    )?;
                ...
                algorithm_oids::Ed25519 => (
                    AlgorithmName::Ed25519,
                    spki_ref.subject_public_key.as_bytes().ok_or(
                        HttpSigError::ParsePublicKeyError("Invalid public key".to_string()),
                    )?,
                ),
                _ => {
...
impl super::VerifyingKey for PublicKey {
    ...
    fn key_id(&self) -> String {
        ...
        let bytes = match self {
            ...
            Self::EcdsaSecp256k1(vk) => vk.to_sec1_bytes().to_vec(),
        };
    ...
    fn alg(&self) -> AlgorithmName {
        match self {
            ...
            Self::EcdsaSecp256k1(_) => AlgorithmName::EcdsaSecp256k1,
        }
    }
}
...
"##;
    ...
    fn test_from_bytes() {
        ...
        let sk =
            SecretKey::from_bytes(AlgorithmName::EcdsaP256Sha256, es256_sk.to_bytes().as_ref())
                .unwrap();
        assert!(matches!(sk, SecretKey::EcdsaP256Sha256(_)));
        let pk_bytes = es256_pk.as_affine().to_bytes();
        let pk =
            PublicKey::from_bytes(AlgorithmName::EcdsaP256Sha256, pk_bytes.as_slice()).unwrap();
